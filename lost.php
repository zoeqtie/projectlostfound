...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_name === "" || $location === "" || $item_date === "" || $contact === "") {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO items (type, item_name, description, location, item_date, contact) VALUES ('lost',?,?,?,?,?)");
        mysqli_stmt_bind_param($stmt, "sssss", $item_name, $description, $location, $item_date, $contact);
        if (mysqli_stmt_execute($stmt)) {
            $message = "บันทึกการแจ้งของหายสำเร็จ";
        } else {
  ...
  <div class="wrapper">
    <div class="card" style="max-width:720px; margin:0 auto;">
      ...
      <form method="post">
        ...
        <div class="form-group"><label>สถานที่คาดว่าหาย</label><input type="text" name="location" required></div>
        <div class="form-group"><label>วันที่หาย</label><input type="date" name="item_date" required></div>
        <div class="form-group"><label>ช่องทางติดต่อ</label><input type="text" name="contact" required></div>
